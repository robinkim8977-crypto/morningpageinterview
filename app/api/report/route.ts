...
export async function POST(request: Request) {
  ...
  const openaiApiBaseUrl = process.env.NODE_ENV === "test"
...
- currentClue는 현재 사실을 판정하는 항목이 아니라, 미래 답변에서 현재의 선택을 살펴볼 수 있는 관심·가치·작은 단서를 180자 이내로 해석하는 항목입니다.
- 인터뷰 답변은 미래의 자신으로 답한 역할극이므로, 답변 속 행동·직업·관계가 현재에도 존재한다고 간주하지 않습니다. '이미 하고 있습니다', '이미 있습니다', '현재 가지고 있습니다'처럼 단정하지 말고, '답변에서 반복해 강조한 기준은 지금의 선택을 살펴볼 단서가 될 수 있습니다'처럼 근거와 가능성을 구분해 씁니다.
...
- 로드맵의 행동은 미래 답변에 나온 직업·회의·팀원·고객·배우자·자녀·특정 공간이 지금도 존재한다고 가정하지 않습니다. 미래의 행동 방식만 가져오고 현재 환경은 요구하지 않는 형태로 바꿉니다. 예를 들어 '회의가 끝난 뒤 결정 이유 기록'은 '최근 내린 선택 하나의 이유 기록'으로, '팀원에게 질문'은 '가까운 대화에서 질문 하나 건네기'로 바꿉니다.
